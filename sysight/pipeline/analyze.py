...
def _build_global_brief(
    ...
    # Bottleneck distribution table (approximate, based on inclusive times)
    ...
    lines.extend([
        ...
        f"| gpu_compute | {_fmt_ms(total_kernel_ns)} | {_fmt_pct(total_kernel_ns, trace_ns)} |",
    ...
    if machine_info.get("device_workload") and int(machine_info.get("active_gpu_count", 0)) > 1:
        lines.extend([
            "",
            "## GPU 负载分布",
    ...
    if nvtx_data and getattr(nvtx_data, "ranges", None):
        lines.extend([
            "",
            "## NVTX Range 分布 Top 5",
    ...
    if overlap_data and overlap_data.compute_kernels > 0 and overlap_data.nccl_kernels > 0:
        lines.extend([
            "",
            "## 计算 / 通信 Overlap",
    ...
    *,
    ...
    if memory_refs:
        for ref in memory_refs:
            if ref.get("label") == "warmup_overview":
                p = ref.get("path", "")
                if p:
                    workspace_path = p
                break
