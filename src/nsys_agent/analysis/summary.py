...
def gpu_summary(prof: Profile, device: int, trim: tuple[int, int] | None = None) -> dict:
    ...
    total_compute_ns = sum(kernel["end"] - kernel["start"] for kernel in kernels)
    nccl_ms = 0.0
    compute_ms = 0.0
        ...
        "top_kernels": [
            {
                ...
                "pct": round(100 * total_ms / (total_compute_ns / 1e6), 1) if total_compute_ns else 0,
            }
...
def auto_commentary(summary: dict) -> str:
    ...
    parts = [
        f"GPU {summary['device']} ran {summary['kernel_count']} kernels over {timing['span_ms']:.0f}ms with {timing['utilization_pct']}% utilization."
    ]
    if top:
        parts.append(
